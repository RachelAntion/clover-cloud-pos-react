...
export default class Register extends React.Component {
    ...
    newOrder(){
        //console.log('newOrder', this.state);
    ...
    chooseSaleMethod(){
        ...
        this.initSettings();
        if(this.state.orderItems.length > 0) {
            if(this.saleMethod === "Vaulted" || this.saleMethod === "PreAuth"){
    ...
    componentWillReceiveProps(newProps) {
        if(newProps.responseFail){
            ...
            this.setState({preAuth: this.store.getPreAuth()});
        }
    }
